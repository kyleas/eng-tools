...
**Current Rocket feature slice (2026-03)**:
- transparent thermal design workflow with regenerative + film cooling controls
- station-wise channel model with pressure-drop-aware automated sizing
- explicit equations/assumptions/provenance surfaced in UI
...
## Phase 5 backend branch kickoff (implemented)

- Introduced `tf-cea` as the ownership boundary for CEA integration.
- Established subprocess adapter contract for using existing CEA backend physics rather than implementing a new solver.
- Delivered initial equilibrium and rocket-performance vertical slice APIs with extension seams for frozen/shifting and combustor model variants.

## Backend progression update: tf-rpa slice

- Added `tf-rpa` as the RPA-style rocket-performance ownership boundary on top of `tf-cea`.
- Delivered first chamber/nozzle orchestration path with explicit combustor and nozzle chemistry assumption modes.
- Preserved explicit unsupported seams for exit-pressure constrained solves and frozen-at-throat chemistry for future phases.

## Rocket app structure milestone

- Established Rocket top-level UI surface with explicit subtab architecture for future growth.
- Delivered first functional Performance subtab backed by `tf-rpa` orchestration.
- Persisted Rocket workspace inputs and selected subtab in project schema.

## Rocket studies milestone

- Added first single-variable Rocket study workflow on top of Performance.
- Added inline results table + shared multi-series plot (Fluid-sweep-style legends/axes behavior) for design-space inspection inside Rocket tab.
- Preserved explicit deferral for multi-variable studies and optimization.

## Rocket propellants usability milestone

- Delivered first searchable propellant preset workflow in Rocket tab.
- Added direct handoff from Propellants subtab to active Performance case.
- Preserved backend-first solve ownership in Performance/Studies with propellant UX layered on top.


## Rocket geometry milestone

- Delivered first functional Rocket > Geometry subtab with first-pass chamber/nozzle sizing.
- Geometry reuses active Performance case assumptions and preserves explicit estimation notes.
- Added canonical engine geometry model + contour-style controls (conical, bell-parabolic, truncated ideal) with persisted geometry configuration for repeatable workflows.


## Rocket thermal milestone

- Delivered first functional Rocket > Thermal subtab with first-pass heat-load estimates.
- Added explicit thermal-model and cooling-mode assumptions with persisted workspace config.
- Added shared multi-series thermal plotting (heat flux/HTC/recovery temperature) with persisted series toggles for quick design iteration.


## Rocket data milestone

- Delivered first functional Rocket > Data assumptions/provenance/diagnostics surface.
- Added explicit provenance taxonomy and warning aggregation across Rocket workflow slices.
- Preserved read-only aggregation design (no duplicate source-of-truth model).
