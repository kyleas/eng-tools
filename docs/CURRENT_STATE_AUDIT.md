...
## Update: CEA backend integration foundation

- Added workspace crate `tf-cea`.
- `tf-cea` defines backend-first domain models for equilibrium and rocket analyses.
- Backend integration is isolated behind `CeaBackend`; current concrete adapter is `CeaProcessAdapter` (subprocess to external CEA bridge executable).
- Vertical slices implemented: equilibrium and rocket API execution paths with adapter integration tests.

## Update: tf-rpa backend slice

- Added workspace crate `tf-rpa` for RPA-style chamber/nozzle backend orchestration.
- `tf-rpa` uses `tf-cea::CeaBackend` and does not reimplement CEA physics.
- First slice returns structured chamber/nozzle performance outputs with explicit placeholder seams for throat/exit station detail pending backend expansion.

## Update: Rocket UI surface

- Added top-level Rocket tab in `tf-ui`.
- Added visible Rocket subtab structure: Performance, Geometry, Thermal, Propellants, Studies, Data.
- Performance subtab is backend-driven through `tf-rpa` and `tf-cea` with clear unsupported-mode error surfacing.
- Added optional project persistence field for Rocket workspace inputs/selected subtab.

## Update: Rocket Studies subtab

- Rocket > Studies is now functional for single-variable sweeps.
- Studies inherit base assumptions and inputs from current Rocket Performance case.
- Results are presented inline as both a table and a shared Rocket multi-series plot aligned with Fluid-style sweep plotting conventions.
- Study configuration is persisted in optional `rocket_workspace.study` project data.

## Update: Rocket Propellants subtab

- Rocket > Propellants is now functional with searchable preset library.
- Applying a preset updates the active Rocket Performance case inputs.
- Propellant selection UI state is persisted in `rocket_workspace.propellants`.


## Update: Rocket Geometry subtab

- Rocket > Geometry is now functional and derived from the current Performance case.
- Supports first-pass sizing from throat diameter or throat area with expansion-ratio-based exit sizing.
- Uses a canonical engine geometry model with axial stations + contour points and supports conical, bell-parabolic, and truncated-ideal nozzle styles (with truncation ratio).
- Provides numeric outputs and an inline canonical-contour preview; assumptions/config persist in `rocket_workspace.geometry`.


## Update: Rocket Thermal subtab

- Rocket > Thermal is now functional and derives from active Performance + Geometry state.
- Supports first-pass chamber/throat/exit heat-flux and peak-load estimation with explicit assumptions.
- Provides shared multi-series plotting (heat flux, gas-side HTC, optional recovery temperature) and persisted thermal series-visibility config in `rocket_workspace.thermal`.


## Update: Rocket Data subtab

- Rocket > Data is now functional as a read-only assumptions/provenance/diagnostics view.
- Distinguishes values by provenance class: native backend, derived in Rust, estimated, unavailable.
- Aggregates active warnings for unsupported modes and first-pass estimate limitations.
## Update: Rocket Thermal Design workflow

- Rocket > Thermal now exposes regenerative, film, and combined cooling modes.
- Thermal inputs now include wall/material properties, coolant properties, channel geometry, and optimizer constraints.
- Backend model computes station-wise heat flux, wall temperatures, coolant state, and pressure drop.
- Automated channel designer iteratively updates channel sizing under `DeltaP_coolant <= DeltaP_max`.
- UI now shows equations, assumptions, provenance-style traces, optimizer iterations, and richer diagnostic plots.
- Fidelity remains first-pass and explicitly documented (no CFD/CHT/manufacturing-final claims).
