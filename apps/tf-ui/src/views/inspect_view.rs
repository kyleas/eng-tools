...
impl InspectView {
    ...
    pub fn show(
        ...
        fluid_view: Option<&mut super::FluidView>,
    ) -> InspectActions {
        let mut actions = InspectActions::default();

        ui.heading("Inspector");

        // Show fluid workspace trace configuration if viewing fluids
        if let Some(fluid_view) = fluid_view {
            ui.label("💧 Fluid Workspace");
            ui.separator();

            egui::ScrollArea::vertical()
                .max_height(600.0)
                .show(ui, |ui| {
                    fluid_view.show_inspector_content(ui);
                });
