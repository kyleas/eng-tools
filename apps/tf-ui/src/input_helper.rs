    ...
    pub fn show_field(
        ...
        ui.horizontal(|ui| {
            ...
            if !state.error.is_empty() {
                ui.label(egui::RichText::new("⚠").color(Color32::RED).size(12.0));
                ui.label(
...
fn format_display_value(si_value: f64, quantity: Quantity) -> String {
    match quantity {
        ...
        Quantity::MassFlow => {
            // Mass flow rate
            if si_value >= 1.0 {
                format!("{:.2} kg/s", si_value)
            } else {
                format!("{:.3} kg/s", si_value)
            }
        }
        Quantity::Length => {
            // Length
            if si_value >= 1.0 {
                format!("{:.3} m", si_value)
            } else if si_value >= 0.01 {
                format!("{:.1} cm", si_value * 100.0)
            } else {
                format!("{:.1} mm", si_value * 1000.0)
            }
        }
        Quantity::DynamicViscosity => {
            if si_value >= 1.0e-3 {
                format!("{:.4} Pa*s", si_value)
            } else {
                format!("{:.2} cP", si_value * 1_000.0)
            }
        }
        Quantity::ThermalConductivity => {
            format!("{:.3} W/m-K", si_value)
        }
        Quantity::HeatTransferCoefficient => {
            if si_value >= 1.0e3 {
                format!("{:.2e} W/m^2-K", si_value)
            } else {
                format!("{:.2} W/m^2-K", si_value)
            }
        }
        Quantity::SpecificHeatCapacity => {
            if si_value >= 1.0e3 {
                format!("{:.1} kJ/kg-K", si_value / 1_000.0)
            } else {
                format!("{:.1} J/kg-K", si_value)
            }
        }
    }
}
