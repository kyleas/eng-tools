...
impl ThermoflowApp {
    ...
    fn new_project(&mut self) {
        ...
        self.rocket_workspace = crate::rocket_workspace::RocketWorkspace::default();
    ...
    fn open_project(&mut self, path: PathBuf) {
        match tf_project::load_yaml(&path) {
            Ok(project) => {
                ...
                if let Some(rocket_workspace_def) = project.rocket_workspace.as_ref() {
                    self.rocket_workspace =
                        crate::rocket_workspace::RocketWorkspace::from_def(rocket_workspace_def);
                } else {
                    self.rocket_workspace = crate::rocket_workspace::RocketWorkspace::default();
                }
    ...
    fn save_project(&mut self) {
        if let Some(project) = self.project.as_mut() {
            if let Some(path) = self.project_path.as_ref() {
                ...
                project.rocket_workspace = Some(self.rocket_workspace.to_def());
        ...
        let inspect_actions = egui::SidePanel::right("inspector")
            ...
            .show(ctx, |ui| {
                ...
                self.inspect_view.show(
                    ...
                    if self.active_view == ViewTab::Fluid {
                        Some(&mut self.fluid_view)
                    } else {
                        None
                    },
                )
        ...
        egui::CentralPanel::default().show(ctx, |ui| {
            ...
            match self.active_view {
                ...
                ViewTab::Fluid => {
                    ...
                    // Handle any curves generated from sweeps in the Fluid workspace
                    if !self.fluid_workspace.pending_curves.is_empty() {
                        // Convert ArbitraryCurveSourceDef to CurveSource for use in plotting
                        for curve_def in self.fluid_workspace.pending_curves.drain(..) {
                            use crate::curve_source::CurveSource;
                            use tf_project::schema::{ArbitraryCurveSourceDef, ValveCharacteristicKindDef};

                            let curve_source = match curve_def {
                                ArbitraryCurveSourceDef::ValveCharacteristic {
                                    component_id,
                                    characteristic,
                                    sample_count,
                                } => {
                                    let char_kind = match characteristic {
                                        ValveCharacteristicKindDef::EffectiveArea => {
                                            crate::curve_source::ValveCharacteristicKind::EffectiveArea
                                        }
                                        ValveCharacteristicKindDef::OpeningFactor => {
                                            crate::curve_source::ValveCharacteristicKind::OpeningFactor
                                        }
                                    };
                                    CurveSource::ValveCharacteristic {
                                        component_id,
                                        characteristic: char_kind,
                                        sample_count,
                                    }
                                }
                                ...
                                ArbitraryCurveSourceDef::FluidPropertySweep {
                                    ...
                                    use crate::curve_source::FluidSweepParameters;
                                    CurveSource::FluidPropertySweep {
                                        x_property,
                                        y_property,
                                        parameters: FluidSweepParameters {
                                            species: parameters.species,
                                            sweep_variable: parameters.sweep_variable,
                                            start_value: parameters.start_value,
                                            end_value: parameters.end_value,
                                            num_points: parameters.num_points,
                                            sweep_type: parameters.sweep_type,
                                            fixed_property_name: parameters.fixed_property.as_ref().map(|f| f.property_name.clone()),
                                            fixed_property_value: parameters.fixed_property.as_ref().map(|f| f.value.clone()),
                                        },
                                    }
                                }
                            };

                            // Add to the active plot if one exists
                            if let Some(active_panel_id) = self.plot_view.get_active_plot_id() {
                                if let Some(panel) = self.plot_view.workspace.panels.get_mut(&active_panel_id) {
                                    panel.series_selection.arbitrary_curves.push(curve_source);
                                }
                            }
                        }
                    }
