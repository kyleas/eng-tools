...
use crate::sweeps::SweepDefinition;
...
impl fmt::Display for SweepError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::TooManyFailures { successful, failed } => {
                write!(
                    f,
...
impl SweepResult {
    ...
    /// Get specific heat at constant pressure array (excluding failed points)
    pub fn cp_j_per_kg_k(&self) -> Vec<f64> {
        self.states
            .iter()
            .filter_map(|s| s.as_ref().map(|state| state.cp_j_per_kg_k))
            .collect()
    }

    /// Get specific heat at constant volume array (excluding failed points)
    pub fn cv_j_per_kg_k(&self) -> Vec<f64> {
        self.states
            .iter()
            .filter_map(|s| s.as_ref().map(|state| state.cv_j_per_kg_k))
            .collect()
    }

    /// Get heat capacity ratio array (excluding failed points)
    pub fn gamma(&self) -> Vec<f64> {
        self.states
            .iter()
            .filter_map(|s| s.as_ref().map(|state| state.gamma))
            .collect()
    }

    /// Get speed of sound array in m/s (excluding failed points)
    pub fn speed_of_sound_m_s(&self) -> Vec<f64> {
        self.states
            .iter()
            .filter_map(|s| s.as_ref().map(|state| state.speed_of_sound_m_s()))
            .collect()
    }
...
mod tests {
    use super::*;
    use crate::{CoolPropModel, SweepType};
    ...
    fn test_temperature_sweep_nitrogen() {
        ...
        let result =
            execute_temperature_sweep_at_pressure(&model, species, &sweep_def, 101_325.0).unwrap();
    ...
    fn test_generic_sweep() {
        ...
        let result =
            execute_generic_sweep(&model, species, FluidInputPair::PT, &sweep_def, 300.0).unwrap();
