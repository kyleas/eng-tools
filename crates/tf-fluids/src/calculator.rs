...
/// Compute saturated liquid state at a given temperature.
pub fn compute_saturated_liquid_at_temperature(
    model: &dyn FluidModel,
    species: Species,
    temperature: f64,
) -> FluidResult<EquilibriumState> {
    ...
    let input = StateInput::TxWithQuality {
        t: k(temperature),
        quality: 0.0,
    };

    compute_with_quality(model, composition, input)
}

/// Compute saturated vapor state at a given temperature.
pub fn compute_saturated_vapor_at_temperature(
    model: &dyn FluidModel,
    species: Species,
    temperature: f64,
) -> FluidResult<EquilibriumState> {
...
/// Compute saturated liquid state at a given pressure.
pub fn compute_saturated_liquid_at_pressure(
    model: &dyn FluidModel,
    species: Species,
    pressure: f64,
) -> FluidResult<EquilibriumState> {
    ...
    let input = StateInput::PxWithQuality {
        p: pa(pressure),
        quality: 0.0,
    };
...
/// Compute saturated vapor state at a given pressure.
pub fn compute_saturated_vapor_at_pressure(
    model: &dyn FluidModel,
    species: Species,
    pressure: f64,
) -> FluidResult<EquilibriumState> {
...
/// Compute state at given pressure/temperature and quality (for two-phase).
pub fn compute_state_with_quality(
    model: &dyn FluidModel,
    species: Species,
    pressure: f64,
    temperature: f64,
    ...
    // Use PxWithQuality for simplicity (quality + pressure determines state in two-phase)
    let input = StateInput::PxWithQuality {
        p: pa(pressure),
        quality,
    };
...
/// Internal helper to compute state with quality information.
fn compute_with_quality(
    model: &dyn FluidModel,
    composition: Composition,
    input: StateInput,
) -> FluidResult<EquilibriumState> {
    // Extract quality if applicable (before moving input)
    let quality_value = match &input {
        StateInput::TxWithQuality { quality, .. } => Some(*quality),
        StateInput::PxWithQuality { quality, .. } => Some(*quality),
        _ => None,
    };
