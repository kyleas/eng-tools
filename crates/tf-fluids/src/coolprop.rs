...
impl CoolPropModel {
    ...
    /// Create a Fluid instance at saturation state with given temperature and quality.
    fn fluid_at_tx(&self, pure: Pure, t_k: f64, quality: f64) -> FluidResult<Fluid> {
        Fluid::from(pure)
            .in_state(FluidInput::temperature(t_k), FluidInput::quality(quality))
            .map_err(|e| FluidError::Backend {
                message: format!(
                    "rfluids error at saturated T={} K, x={}: {}",
                    t_k, quality, e
                ),
            })
    }

    /// Solve for saturation temperature at a given pressure.
    fn solve_t_sat_from_p(&self, pure: Pure, p_pa: f64) -> FluidResult<f64> {
        // For saturation, use quality = 0.5 (dew point) but the temperature should be unique
        // at saturation regardless of quality within (0,1)
        let mut fluid = Fluid::from(pure)
            .in_state(FluidInput::pressure(p_pa), FluidInput::quality(0.5))
            .map_err(|e| FluidError::Backend {
                message: format!("rfluids error finding saturation T at P={} Pa: {}", p_pa, e),
            })?;

        let t_k = fluid.temperature().map_err(|e| FluidError::Backend {
            message: format!("rfluids error getting saturation temperature: {}", e),
        })?;
...
impl FluidModel for CoolPropModel {
    ...
    fn state(&self, input: StateInput, comp: Composition) -> FluidResult<ThermoState> {
        ...
        match input {
            ...
            StateInput::TxWithQuality { t, quality } => {
                validation::validate_temperature(t)?;
                if !quality.is_finite() || !(0.0..=1.0).contains(&quality) {
                    return Err(FluidError::NonPhysical {
                        what: "quality must be between 0.0 and 1.0",
                    });
                }

                // For saturation state at temperature with quality, create fluid at sat state
                let mut fluid = self.fluid_at_tx(pure, t.value, quality)?;

                // Get the saturation pressure at this temperature
                let p_pa = fluid.pressure().map_err(|e| FluidError::Backend {
                    message: format!("rfluids error getting saturation pressure: {}", e),
                })?;
                let p = tf_core::units::pa(p_pa);

                // Return state at saturation (P,T determined by saturation curve)
                ThermoState::from_pt(p, t, comp)
            }
            StateInput::PxWithQuality { p, quality } => {
                validation::validate_pressure(p)?;
                if !quality.is_finite() || !(0.0..=1.0).contains(&quality) {
                    return Err(FluidError::NonPhysical {
                        what: "quality must be between 0.0 and 1.0",
                ...
                // For saturation state at pressure with quality, find saturation temperature
                let t_k = self.solve_t_sat_from_p(pure, p.value)?;
                let t = k(t_k);
    ...
    fn dynamic_viscosity(&self, state: &ThermoState) -> FluidResult<f64> {
        let species = state
            .composition()
            .is_pure()
            .ok_or(FluidError::NotSupported {
        ...
        let mu = fluid.dynamic_viscosity().map_err(|e| FluidError::Backend {
            message: format!("rfluids error getting dynamic viscosity: {}", e),
        })?;
        if !mu.is_finite() || mu <= 0.0 {
            return Err(FluidError::NonPhysical {
                what: "dynamic viscosity must be positive and finite",
            });
        }
        Ok(mu)
    }

    fn thermal_conductivity(&self, state: &ThermoState) -> FluidResult<f64> {
        let species = state
            .composition()
            .is_pure()
            .ok_or(FluidError::NotSupported {
        ...
        let mut fluid = self.fluid_at_pt(pure, p_pa, t_k)?;
        let k = fluid.conductivity().map_err(|e| FluidError::Backend {
            message: format!("rfluids error getting thermal conductivity: {}", e),
        })?;
        if !k.is_finite() || k <= 0.0 {
            return Err(FluidError::NonPhysical {
                what: "thermal conductivity must be positive and finite",
            });
        }
        Ok(k)
    }
