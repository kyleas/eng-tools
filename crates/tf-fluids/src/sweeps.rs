...
mod tests {
    ...
    fn sweep_from_text() {
        let sweep =
            SweepDefinition::from_text("300K", "400K", Quantity::Temperature, 5, SweepType::Linear)
                .unwrap();
    ...
    fn reject_invalid_point_count() {
        let result =
            SweepDefinition::from_text("300K", "400K", Quantity::Temperature, 1, SweepType::Linear);
    ...
    fn reject_identical_bounds() {
        let result =
            SweepDefinition::from_text("300K", "300K", Quantity::Temperature, 5, SweepType::Linear);
        assert!(result.is_err());
    }
}
