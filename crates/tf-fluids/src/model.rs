...
pub trait FluidModel: Send + Sync {
    ...
    /// Compute dynamic viscosity [Pa*s] at the given state.
    ///
    /// Backends may return `NotSupported` if transport properties are unavailable.
    fn dynamic_viscosity(&self, _state: &ThermoState) -> FluidResult<f64> {
        Err(FluidError::NotSupported {
            what: "dynamic viscosity",
        })
    }

    /// Compute thermal conductivity [W/(m*K)] at the given state.
    ///
    /// Backends may return `NotSupported` if transport properties are unavailable.
    fn thermal_conductivity(&self, _state: &ThermoState) -> FluidResult<f64> {
        Err(FluidError::NotSupported {
