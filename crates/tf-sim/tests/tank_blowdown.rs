...
impl FluidModel for SimpleIdealGasModel {
    ...
    fn state(&self, input: StateInput, comp: Composition) -> tf_fluids::FluidResult<ThermoState> {
        let (p, t) = match input {
            ...
            StateInput::TxWithQuality { t, quality: _ } => {
                // For ideal gas, quality doesn't affect state
                (pa(101_325.0), t)
            }
            StateInput::PxWithQuality { p, quality: _ } => {
                // For ideal gas, quality doesn't affect state
                (
                    p,
                    Temperature::new::<uom::si::thermodynamic_temperature::kelvin>(300.0),
                )
            }
        };
