...
pub enum Quantity {
    ...
    ThermalConductivity,
    /// Heat transfer coefficient (canonical: W/(m^2*K))
...
impl fmt::Display for Quantity {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::MassFlow => write!(f, "Mass Flow"),
            Self::Length => write!(f, "Length"),
            Self::DynamicViscosity => write!(f, "Dynamic Viscosity"),
            Self::ThermalConductivity => write!(f, "Thermal Conductivity"),
            Self::HeatTransferCoefficient => write!(f, "Heat Transfer Coefficient"),
            Self::SpecificHeatCapacity => write!(f, "Specific Heat Capacity"),
        }
    }
}
...
pub fn parse_quantity(raw_text: &str, quantity: Quantity) -> Result<f64, UnitError> {
    ...
    match quantity {
        ...
        Quantity::MassFlow => parse_mass_flow(trimmed),
        Quantity::Length => parse_length(trimmed),
        Quantity::DynamicViscosity => parse_dynamic_viscosity(trimmed),
        Quantity::ThermalConductivity => parse_thermal_conductivity(trimmed),
        Quantity::HeatTransferCoefficient => parse_heat_transfer_coefficient(trimmed),
        Quantity::SpecificHeatCapacity => parse_specific_heat_capacity(trimmed),
    }
}
...
fn parse_quality(input: &str) -> Result<f64, UnitError> {
    ...
    let quality = if trimmed.ends_with('%') {
        ...
        let percent: f64 = num_str.parse().map_err(|_| {
            UnitError::ParseError(format!("Could not parse quality from '{}'", input))
        })?;
        percent / 100.0
    } else {
    ...
    let value = if trimmed.ends_with('%') {
        ...
        let percent: f64 = num_str.parse().map_err(|_| {
            UnitError::ParseError(format!(
                "Could not parse dimensionless value from '{}'",
                input
            ))
        })?;
        percent / 100.0
...
fn parse_mass_flow(input: &str) -> Result<f64, UnitError> {
    ...
    let kg_per_s = match unit.to_lowercase().as_str() {
        "kg/s" | "kg" => value,
        "g/s" => value / 1000.0,
        "lbm/s" | "lbs/s" => value * 0.453_592,
        "" => value,
        _ => {
            return Err(UnitError::UnknownUnit {
                unit: unit.to_string(),
                quantity: "Mass Flow".to_string(),
            });
        }
    };
...
/// Parse length in various units, return meters.
fn parse_length(input: &str) -> Result<f64, UnitError> {
    let (value, unit) = split_value_and_unit(input)?;
...
fn parse_dynamic_viscosity(input: &str) -> Result<f64, UnitError> {
    let (value, unit) = split_value_and_unit(input)?;
    let pa_s = match unit.to_lowercase().as_str() {
        "pa*s" | "pa s" | "pas" => value,
        "mpa*s" | "mpa s" => value * 1e-3,
        "cp" => value * 1e-3,
        "" => value,
        _ => {
            return Err(UnitError::UnknownUnit {
                unit: unit.to_string(),
                quantity: "Dynamic Viscosity".to_string(),
            });
        }
    };
    if pa_s <= 0.0 {
        return Err(UnitError::OutOfRange {
            value: pa_s,
            reason: "Dynamic viscosity must be positive".to_string(),
        });
    }
    Ok(pa_s)
}

fn parse_thermal_conductivity(input: &str) -> Result<f64, UnitError> {
    let (value, unit) = split_value_and_unit(input)?;
    let w_m_k = match unit.to_lowercase().as_str() {
        "w/m-k" | "w/(m*k)" | "w/mk" => value,
        "kw/m-k" | "kw/(m*k)" => value * 1e3,
        "btu/(hr*ft*f)" | "btu/(h*ft*f)" => value * 1.730_735,
        "" => value,
        _ => {
            return Err(UnitError::UnknownUnit {
                unit: unit.to_string(),
                quantity: "Thermal Conductivity".to_string(),
            });
        }
    };
    if w_m_k <= 0.0 {
...
fn parse_heat_transfer_coefficient(input: &str) -> Result<f64, UnitError> {
    let (value, unit) = split_value_and_unit(input)?;
    let w_m2_k = match unit.to_lowercase().as_str() {
        "w/m^2-k" | "w/m2-k" | "w/(m^2*k)" | "w/(m2*k)" => value,
        "kw/m^2-k" | "kw/m2-k" | "kw/(m^2*k)" | "kw/(m2*k)" => value * 1e3,
        "btu/(hr*ft^2*f)" | "btu/(h*ft^2*f)" => value * 5.678_263,
        "" => value,
        _ => {
            return Err(UnitError::UnknownUnit {
                unit: unit.to_string(),
                quantity: "Heat Transfer Coefficient".to_string(),
            });
        }
    };
    if w_m2_k <= 0.0 {
        return Err(UnitError::OutOfRange {
            value: w_m2_k,
            reason: "Heat transfer coefficient must be positive".to_string(),
        });
    }
    Ok(w_m2_k)
}

fn parse_specific_heat_capacity(input: &str) -> Result<f64, UnitError> {
    let (value, unit) = split_value_and_unit(input)?;
    let j_kg_k = match unit.to_lowercase().as_str() {
        "j/(kg*k)" | "j/(kg k)" | "j/kg-k" => value,
        "kj/(kg*k)" | "kj/(kg k)" | "kj/kg-k" => value * 1e3,
        "btu/(lbm*r)" | "btu/(lbm r)" => value * 4_186.8,
        "" => value,
        _ => {
            return Err(UnitError::UnknownUnit {
                unit: unit.to_string(),
                quantity: "Specific Heat Capacity".to_string(),
            });
        }
    };
    if j_kg_k <= 0.0 {
        return Err(UnitError::OutOfRange {
            value: j_kg_k,
            reason: "Specific heat capacity must be positive".to_string(),
        });
    }
    Ok(j_kg_k)
}
